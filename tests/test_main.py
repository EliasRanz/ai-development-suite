...
@patch('comfy_launcher.__main__.TrayManager') # Patch TrayManager
class TestMainExecution(unittest.TestCase):
    ...
    def test_main_orchestration_and_shutdown(
            self, mock_settings_param,                # from @patch('...settings')
            mock_setup_logger_param,            # from @patch('...setup_launcher_logger')
            mock_rotate_cleanup_param,          # from @patch('...rotate_and_cleanup_logs')
            MockServerManager_param,            # from @patch('...ServerManager')
            MockGUIManager_param,               # from @patch('...GUIManager')
            mock_time_sleep_param,              # from @patch('...time.sleep')
            mock_app_logic_thread_func_param,   # from @patch('...app_logic_thread_func')
            MockTrayManager_class_level_param ):# From class decorator (comes last in signature for method mocks)
        """
        ...
        mock_setup_logger_param.return_value = mock_logger_instance

        mock_gui_instance = MockGUIManager_param.return_value
        mock_server_instance = MockServerManager_param.return_value
        mock_tray_instance = MockTrayManager_class_level_param.return_value

        # Configure settings
        mock_settings_param.DEBUG = False
        mock_settings_param.APP_NAME = "TestApp"
        mock_settings_param.LOG_DIR = Path("/fake/logs")
        mock_settings_param.MAX_LOG_FILES = 3
        mock_settings_param.MAX_LOG_AGE_DAYS = 5
        mock_settings_param.COMFYUI_PATH = Path("/fake/comfyui")
        mock_settings_param.PYTHON_EXECUTABLE = Path("/fake/python")
        mock_settings_param.HOST = "127.0.0.1"
        mock_settings_param.PORT = 8188
        mock_settings_param.ASSETS_DIR = Path("/fake/assets")
        mock_settings_param.WINDOW_WIDTH = 800
        mock_settings_param.WINDOW_HEIGHT = 600
        mock_settings_param.EFFECTIVE_CONNECT_HOST = mock_settings_param.HOST
        ...
        with patch('comfy_launcher.__main__.threading.Thread') as MockAppThread:
            ...
            mock_rotate_cleanup_param.assert_called_once_with(
                mock_settings_param.LOG_DIR, mock_settings_param.MAX_LOG_FILES, mock_settings_param.MAX_LOG_AGE_DAYS
            )
            mock_setup_logger_param.assert_called_once_with(
                mock_settings_param.LOG_DIR, mock_settings_param.DEBUG
            )

            # 2. Manager instantiation
            MockServerManager_param.assert_called_once_with(
                comfyui_path=mock_settings_param.COMFYUI_PATH,
                python_executable=mock_settings_param.PYTHON_EXECUTABLE,
                listen_host=mock_settings_param.HOST,
                connect_host=mock_settings_param.EFFECTIVE_CONNECT_HOST,
                port=mock_settings_param.PORT,
                logger=mock_logger_instance
            )
            MockGUIManager_param.assert_called_once_with(
                app_name=mock_settings_param.APP_NAME,
                window_width=mock_settings_param.WINDOW_WIDTH,
                window_height=mock_settings_param.WINDOW_HEIGHT,
                connect_host=mock_settings_param.EFFECTIVE_CONNECT_HOST,
                port=mock_settings_param.PORT,
                assets_dir=mock_settings_param.ASSETS_DIR,
                logger=mock_logger_instance,
                server_manager=mock_server_instance
            )
            MockTrayManager_class_level_param.assert_called_once_with(
                app_name=mock_settings_param.APP_NAME,
                assets_dir=mock_settings_param.ASSETS_DIR,
                logger=mock_logger_instance
            )
            ...
            expected_server_log_path = mock_settings_param.LOG_DIR / "server.log"
            ...
            self.assertEqual(launcher_main_module.tray_manager_instance, mock_tray_instance) # Check tray instance
            ...
            # 7. Final logging
            mock_logger_instance.info.assert_any_call(f"MAIN THREAD: {mock_settings_param.APP_NAME} has exited cleanly.")
            # 8. Thread join
    ...
    def test_app_logic_thread_func_successful_run(self, mock_threading_Thread_actual, # from @patch('...threading.Thread')
                                                mock_time_sleep_actual,       # from @patch('...time.sleep')
        ...
        # time.sleep(0.5) - mock_time_sleep_actual handles this
        ...
        mock_threading_Thread_actual.assert_called_once_with(
            target=mock_gui_manager.redirect_when_ready_loop, daemon=True
        )
        mock_threading_Thread_actual.return_value.start.assert_called_once()
    ...
    @patch('comfy_launcher.__main__.time.sleep', return_value=None)
    def test_app_logic_thread_func_gui_timeout(self, mock_time_sleep_param, # from @patch('...time.sleep')
                                               MockTrayManager_class_level_param): # From class decorator
    ...
    def test_app_logic_thread_func_server_start_fails(self, mock_threading_Thread_actual, # from @patch('...threading.Thread')
                                                      mock_time_sleep_actual,       # from @patch('...time.sleep')
                                                      MockTrayManager_class_level_param): # From class decorator
        ...
        mock_threading_Thread_actual.assert_not_called() # Redirect loop should not start
